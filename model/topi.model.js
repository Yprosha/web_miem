...
class TopiModel {
    ...
    async create(place, name, photo_link, rate, film_link, description) {
        const query = await pool.query('INSERT INTO topi (place, name, photo_link, rate, film_link, description)' +
            ' VALUES ($1, $2, $3, $4, $5, $6) RETURNING id', [place, name, photo_link, rate, film_link, description]);
        return query.rows[0].id;
    }

    async getAllFilm(pageSize, offset) {
        const query = await pool.query('SELECT * FROM topi ORDER BY place ASC LIMIT $1 OFFSET $2;',
            [pageSize, offset]);
        return query.rows;
    }

    async getOneFilm(film_id) {
        const query = await pool.query('SELECT * FROM topi where id = $1;',
            [film_id]);
        return query.rows[0];
    }

    async update(id, place, name, photo_link, rate, film_link, description) {
        let query = 'update topi set place = $1, name = $2, photo_link = $3, rate = $4,' +
    ...
    async delete(id) {
        let query = 'delete from topi where id = $1;';
